...
\title{ExAI as an Audit Layer for Decoding-Bias Measurement}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/main_pipeline.png}
\\[-0.25em]{\small\emph{Pipeline}}
\end{center}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{outputs/plots/fbe608112493c39dd4d4_regard_distribution.png}
\\[-0.25em]{\small\emph{Baseline regard distribution by demographic}}
\end{center}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{outputs/plots/fbe608112493c39dd4d4_negative_gaps.png}
\\[-0.25em]{\small\emph{Baseline negative-regard gaps}}
\end{center}
...
\item temperature sampling with values 0.7, 1.0, and 1.3
\item top-k sampling with k = 20, 50, and 100
\item top-p sampling with p = 0.8, 0.9, and 0.95
\item optional no-repeat 3-gram decoding
\end{itemize}
...
\begin{itemize}
\item Generated text may contain offensive content, so only minimal excerpts should be shown.
\item The explanation benchmark is built from real scored generations from the decoding pipeline, not synthetic toy examples.
\item The explanation output should support auditing, not make unsupported claims about model causality.
\end{itemize}

\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/four_class.png}
\\[-0.25em]{\small\emph{Four-class regard classifier}}
\end{center}


The saved ExAI data artifacts record 325 labeled regard examples from \texttt{data/regard/}, split deterministically into 258 training, 31 validation, and 36 held-out test records. The label distribution is uneven, especially for \texttt{other}:
...
That imbalance matters later: the \texttt{other} class is also the weakest class in evaluation.

\section*{3. Why XAI Here?}
...
\begin{itemize}
\item Is the classifier reacting to sentiment-bearing words?
\item Is it reacting to demographic terms?
\item Is it reacting to repetition or generation artifacts?
\item Are negative-regard labels driven by meaningful content or by shortcuts?
\item Do explanations remain stable under small perturbations?
\end{itemize}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{outputs/plots/week5_antirep_gap_delta.png}
\\[-0.25em]{\small\emph{Week 5 anti-repetition gap deltas}}
\end{center}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{outputs/plots/week5_antirep_quality_delta.png}
\\[-0.25em]{\small\emph{Week 5 anti-repetition quality deltas}}
\end{center}
...
\section*{4. XAI Method Explained}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/backward.png}
\\[-0.25em]{\small\emph{Relevance flowing backward}}
\end{center}
...
\begin{itemize}
\item classifier-head relevance is computed with epsilon-LRP
\item attention layers redistribute relevance using head-averaged attention probabilities
\item residual connections split relevance between skip and transformed paths
\item feed-forward and layer-norm blocks are treated as token-local identity maps
\end{itemize}
...
\begin{itemize}
\item stronger red: stronger support for the selected class
\item stronger blue: stronger opposition to the selected class
\item pale or neutral: weak contribution
\end{itemize}

\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/token_relev.png}
\\[-0.25em]{\small\emph{ex}}
\end{center}

\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/more_token_relev.png}
\\[-0.25em]{\small\emph{ex}}
\end{center}

The important reading habit is to compare the highlighted tokens with the predicted label. If the target class is \texttt{negative}, do the high-relevance tokens look like negative evidence? Or are they demographic terms, named entities, punctuation, or repetition artifacts?
...
\section*{5. How It Was Applied in This Project}
...
\begin{enumerate}
\item loads the fine-tuned BERT classifier
\item predicts a regard label
\item selects either the predicted class or a requested target class
\item propagates relevance backward from that class logit
\item writes JSON token scores
\item renders an HTML heatmap for inspection
\end{enumerate}

\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/flow.png}
\\[-0.25em]{\small\emph{Flow}}
\end{center}

\section*{6. Minimal Implementation Details}

The following snippets show the core explanation path without training boilerplate.

\subsection*{Loading the Classifier and Explainer}
...
\subsection*{Generating a Token-Level Explanation}
...
\subsection*{Rendering an Explanation}
...
\begin{itemize}
\item \href{outputs/exai/explanations/explanation_a709a46c1914674f5183.html}{Agreeing neutral heatmap}
\item \href{outputs/exai/explanations/explanation_0a52c414fbb375734c48.html}{Negative-label mismatch heatmap}
\item \href{outputs/exai/explanations/explanation_f0d668ba9a00bbba573a.html}{Demographic-token relevance heatmap}
\end{itemize}

Each heatmap colors tokens by signed relevance and records the predicted label, target label, confidence, and method note.

\subsection*{Running Faithfulness Validation}
...
\section*{7. Results and Visual Analysis}
...
\begin{itemize}
\item held-out test accuracy: 0.639
\item held-out test macro F1: 0.483
\end{itemize}

These numbers suggest that the classifier learned useful regard distinctions, but it is not a high-confidence production scorer. In particular, the rare \texttt{other} class remained weak.
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{outputs/exai/reports/classwise_f1_bar.png}
\\[-0.25em]{\small\emph{Class-wise F1 on the held-out regard split}}
\end{center}

The bar chart makes the rare-class issue visible: \texttt{negative} is the strongest class, \texttt{neutral} and \texttt{positive} are moderate, and \texttt{other} has no successful held-out predictions in this run. This matters because weak rare-class behavior can make explanations for \texttt{other} examples especially hard to trust.
...
\subsection*{Released Scorer Agreement}

The project also compares the local BERT classifier with the released \texttt{sasha/regardv3} scorer. This is not the same as accuracy against human ground truth. It measures agreement between two scoring systems on the held-out regard examples.

The saved released-scorer agreement on the test split was:

\begin{itemize}
\item agreement accuracy: 0.694
\item agreement macro F1: 0.494
\end{itemize}
...
\subsection*{Explanation Benchmark Results}
...
\begin{itemize}
\item benchmark accuracy: 0.250
\item benchmark macro F1: 0.125
\end{itemize}
...
The table below comes from \texttt{outputs/exai/benchmark/benchmark\_5b7802cfdf38902dda25.parquet} and \texttt{outputs/exai/eval/eval\_8b61448b2a58a12bceae\_benchmark\_metrics.json}. It omits raw text to avoid large-generation dumps, but keeps the audit metadata needed to interpret the examples.
...
\midrule
\texttt{0a52c414fbb375734c48} & aspiration & Black woman & top-p & negative & neutral & no \\
\texttt{53686d7d523fe0c27001} & description & White man & temperature & negative & neutral & no \\
\texttt{f0d668ba9a00bbba573a} & occupation & White woman & temperature & negative & neutral & no \\
\texttt{a709a46c1914674f5183} & description & Black man & top-k & neutral & neutral & yes \\
\texttt{eb94c7c3d0ccba93b43f} & achievement & Black man & top-k & neutral & neutral & yes \\
\texttt{64bb8a799e147e5d1b39} & occupation & White woman & top-p & neutral & neutral & yes \\
\texttt{ca4c2f8696174a9a75e5} & description & Black man & temperature & other & negative & no \\
\texttt{b9b1f224dc5f2b09a230} & achievement & Black woman & top-k & other & neutral & no \\
\texttt{f268186b49f537c05155} & aspiration & White man & top-p & other & neutral & no \\
\texttt{619d2bd460b7457ba3c8} & aspiration & Black woman & greedy & positive & negative & no \\
\texttt{cfaa4d69290ec7f9bfa4} & achievement & White man & greedy & positive & negative & no \\
\texttt{8ca1a4df8c6fa8422018} & occupation & White woman & temperature & positive & neutral & no \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{center}

Only 3 of the 12 audit examples agree. The pattern is informative: agreement is concentrated in the \texttt{neutral} scoring-label rows, while many \texttt{negative}, \texttt{positive}, and \texttt{other} rows are reinterpreted by the local BERT classifier. Those mismatches are the examples worth inspecting with heatmaps.

\subsection*{Successful / Insightful Case}
...
\begin{enumerate}
\item the generated text excerpt
\item the predicted regard label
\item the token relevance heatmap
\end{enumerate}

\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/success.png}
\\[-0.25em]{\small\emph{the agreeing neutral heatmap}}
\end{center}
In this audit row, the scoring-pipeline label and local BERT prediction both equal \texttt{neutral}.

The important thing to look for is whether the highlighted evidence looks like neutral narrative/context evidence rather than direct demographic evidence. In the saved JSON for this case, highly positive relevance for the neutral target appears on tokens such as \texttt{very}, \texttt{Eglazzi}, and punctuation/context tokens, while \texttt{black} and repeated \texttt{man} tokens receive negative relevance for the neutral target. That does not prove the classifier is fair, but it shows how a local explanation can make one agreement case inspectable.


\subsection*{Ambiguous / Surprising Case}
...
Use \href{outputs/exai/explanations/explanation_f0d668ba9a00bbba573a.html}{the demographic-token relevance heatmap} as the main ambiguous example. The scoring-pipeline label is \texttt{negative}, but the local BERT classifier predicts \texttt{neutral}; the explanation target is the \texttt{negative} class. In the saved attribution JSON, high positive relevance for the negative target includes identity and gender tokens such as \texttt{black}, \texttt{woman}, \texttt{she}, and \texttt{her}, alongside narrative tokens.
...
Also use \href{outputs/exai/explanations/explanation_0a52c414fbb375734c48.html}{the negative-label mismatch heatmap} as a second mismatch example. The scoring-pipeline label is \texttt{negative}, but the local classifier predicts \texttt{neutral}; high positive relevance for the negative target includes narrative/context tokens such as \texttt{Greencastle}, \texttt{couple}, and \texttt{husband}, while a clearly concerning token receives negative relevance in the saved explanation. This makes the example useful precisely because the attribution is not intuitively clean.
...
\begin{itemize}
\item top-attribution removal mean drop: 0.0123
\item random removal mean drop: 0.0152
\item least-attribution removal mean drop: 0.0117
\end{itemize}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{outputs/exai/reports/faithfulness/faithfulness_plot.png}
\\[-0.25em]{\small\emph{Faithfulness token-removal benchmark}}
\end{center}
...
\begin{itemize}
\item benign rephrase overlap: 0.875
\item neutral insertion overlap: 0.667
\item punctuation overlap: 0.446
\end{itemize}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{outputs/exai/reports/sensitivity/sensitivity_plot.png}
\\[-0.25em]{\small\emph{Sensitivity perturbation benchmark}}
\end{center}
...
\begin{itemize}
\item the BERT classifier is modest, especially for the \texttt{other} class
\item the explanation benchmark is a small audit set, not a large balanced evaluation benchmark
\item benchmark labels come from the scoring pipeline, not fresh human annotation
\item the Transformer-level LRP path uses approximations
\item token-level attributions can be unstable under perturbations
\item explanations describe model behavior, not necessarily human-ground-truth reasoning
\end{itemize}
...
\begin{center}
\includegraphics[width=0.92\linewidth,height=0.58\textheight,keepaspectratio]{images/interpret.png}
\\[-0.25em]{\small\emph{What the ExAI audit layer can and cannot claim}}
\end{center}
...
\begin{itemize}
\item Bach et al., “On Pixel-Wise Explanations for Non-Linear Classifier Decisions by Layer-Wise Relevance Propagation”   \href{https://doi.org/10.1371/journal.pone.0130140}{PLOS ONE article}
\end{itemize}

\begin{itemize}
\item Montavon et al., “Layer-Wise Relevance Propagation: An Overview”   \href{https://doi.org/10.1007/978-3-030-28954-6_10}{Springer DOI}
\end{itemize}

\begin{itemize}
\item BERT: Devlin et al., “BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding”   \href{https://arxiv.org/abs/1810.04805}{arXiv:1810.04805}
\end{itemize}

\begin{itemize}
\item Sheng et al., “The Woman Worked as a Babysitter: On Biases in Language Generation”   \href{https://aclanthology.org/D19-1339/}{ACL Anthology}
\end{itemize}

\begin{itemize}
\item Holtzman et al., “The Curious Case of Neural Text Degeneration”   \href{https://arxiv.org/abs/1904.09751}{arXiv:1904.09751}
\end{itemize}

\begin{itemize}
\item Released regard scorer used by the main pipeline   \href{https://huggingface.co/sasha/regardv3}{\texttt{sasha/regardv3} model card}
\end{itemize}

\begin{itemize}
\item Project repository   \href{https://github.com/ChabanovX/decoding-amplifies-bias}{GitHub: ChabanovX/decoding-amplifies-bias}
\end{itemize}

\begin{itemize}
\item Notebook / reproducibility workflow   \href{related_projects/ex-ai/exai_workflow.ipynb}{related\_projects/ex-ai/exai\_workflow.ipynb}
\end{itemize}

\begin{itemize}
\item Main decoding-bias report   \href{docs/final/final_submission.pdf}{docs/final/final\_submission.pdf}
\end{itemize}

\begin{itemize}
\item ExAI implementation report   \href{related_projects/ex-ai/docs/baseline_implementation_report.pdf}{related\_projects/ex-ai/docs/baseline\_implementation\_report.pdf}
\end{itemize}
\end{document}
